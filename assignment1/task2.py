...
if __name__ == '__main__':
  ...
  # Task 2
  horizon = 30000
  p2 = np.arange(0, 0.95, 0.05)
  p1 = [
    np.ones_like(p2) * 0.9, # part A
    p2 + 0.1 # part B
  ]
  regret = [
    task2(UCB, horizon, p1[0], p2), # part A
    task2(UCB, horizon, p1[1], p2), # part B
    task2(KL_UCB, horizon, p1[1], p2) # part B
  ]
  print(regret)
  for i in range(3):
    plt.title("Variation of regret with p2" + [" for UCB", " for KL UCB"][i//2])
    plt.xlabel("p2")
    plt.ylabel("Regret")
    plt.plot(p2, regret[i])
    plt.savefig(f"task2-{['A', 'B-UCB', 'B-KL_UCB'][i]}.png")
    plt.clf()
